...
import supabase from "../../supabaseClient"; // kept only for photo upload to Supabase Storage
import "./Profile.css";
...
function Profile({ user, onBack, onLogout, onUpdateUser }: ProfileProps) {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      } catch (e) { /* silently fail — profile may not exist yet */ }
    };
    loadProfile();
  }, [user.id, user.name]);
  ...
  const handleEditProfile = async () => {
    ...
    try {
      ...
      onUpdateUser({ ...user, name: fullName });
    } catch (e: any) {
  ...
  // ── Upload Photo — kept on Supabase Storage ──
  ...
  const handleUploadPhoto = async () => {
    ...
    // Upload to Supabase Storage (photo upload kept here; backend doesn't support file storage)
    ...
    // Save URL to profiles via Spring Boot backend
    try {
      await api.upsertProfile({
        id: user.id,
        full_name: fullName,
      ...
      setError(e?.message || "Failed to save photo URL.");
    }
    setLoading(false);
  };
        ...
        {activeTab === "profile" && (
          <div className="profile-card fade-up">
            ...
            <div className="form-group">
              ...
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Theodore Narsico" />
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              <label>Address</label>
              <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="123 Filipino St, Cebu City" />
            </div>
            <div className="form-group">
              <label>Email</label>
        ...
        {activeTab === "photo" && (
          <div className="profile-card fade-up">
            ...
            <div className="form-group">
              ...
              <input type="file" accept=".jpg,.jpeg,.png" onChange={handlePhotoChange} className="file-input" />
            </div>
